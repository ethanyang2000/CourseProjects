...
class Graph:
    ...
    def max_degree_nodes(self) -> dict:
        ...
        counts = {}
        for edge in self.edges:
            if edge[0] not in counts.keys():
                counts[edge[0]] = 0
            if edge[1] not in counts.keys():
                counts[edge[1]] = 0
            counts[edge[0]] += 1
            counts[edge[1]] += 1
        maxVal = max(list(counts.values()))
        res = {}
...
class  TMDBAPIUtils:
    ...
    def get_movie_cast(self, movie_id:str, limit:int=None, exclude_ids:list=None) -> list:
        ...
        conn.request("GET", "/3/movie/{}/credits?language=en-US&api_key={}".format(movie_id, self.api_key))
        res = json.loads(conn.getresponse().read())
        
        cast = res.get("cast", [])
        for actor in cast:
            if limit is not None and actor['order'] >= limit:
                break
    ...
    def get_movie_credits_for_person(self, person_id:str, vote_avg_threshold:float=None)->list:
        ...
        ans = []
        conn = http.client.HTTPSConnection("api.themoviedb.org")
        conn.request("GET", "/3/person/{}/credits?language=en-US&api_key={}".format(person_id, self.api_key))
        res = json.loads(conn.getresponse().read())
        
        cast = res.get("cast", [])
        for movie in cast:
            if vote_avg_threshold is not None and movie['vote_average'] < vote_avg_threshold:
                continue
            ans.append(movie)
...
def exec(nodes: [], utils: TMDBAPIUtils, graph: Graph):
    new_nodes = []
    for node in nodes:
        movies = utils.get_movie_credits_for_person(node['id'], 8.0)
        for movie in movies:
            casts = utils.get_movie_cast(movie['id'], 3)
            for cast in casts:
                if not graph.exist(cast['id']):
                    graph.add_node(cast['id'], cast['name'])
                    new_nodes.append(cast)
                graph.add_edge(node['id'], cast['id'])
...
if __name__ == "__main__":
    ...
    tmdb_api_utils = TMDBAPIUtils(api_key="6d0126e8132057155309a6e602cfcd27")
    new_nodes = exec([{'id':'2975'}], tmdb_api_utils, graph)
    # call functions or place code here to build graph (graph building code not graded)
    # Suggestion: code should contain steps outlined above in BUILD CO-ACTOR NETWORK

    new_nodes = exec(new_nodes, tmdb_api_utils, graph)
    new_nodes = exec(new_nodes, tmdb_api_utils, graph)
    graph.write_edges_file()
    graph.write_nodes_file()
