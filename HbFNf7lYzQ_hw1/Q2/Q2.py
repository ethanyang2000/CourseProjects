...
class HW2_sql():
    ...
    def part_aii_1(self,connection,path):
        ...
        for line in file:
            ...
            connection.execute(sql, (line[0], line[1], line[2]))
        #####################################################################
    ...
    def part_aii_2(self,connection, path):
        ...
        for line in file:
            ...
            connection.execute(sql, (line[0], line[1], line[2], line[3], line[4]))
        ######################################################################
    ...
    def part_c(self,connection):
        ############### EDIT SQL STATEMENT ###################################
        part_c_sql = "SELECT printf(\"%.2f\", (COUNT(*) * 100.0) / (SELECT COUNT(*) FROM movies)) FROM movies WHERE score BETWEEN 7.0 AND 20.0"
        ######################################################################
    ...
    def part_e(self,connection):
        ############### EDIT SQL STATEMENT ###################################
        part_e_sql = "SELECT movies.title AS movie_title, printf(\"%.2f\", movies.score), COUNT(*) AS cast_count FROM movies JOIN movie_cast ON movies.id = movie_cast.movie_id GROUP BY movies.id ORDER BY movies.score DESC, cast_count ASC, movie_title ASC LIMIT 5"
        ######################################################################
    ...
    def part_f(self,connection):
        ############### EDIT SQL STATEMENT ###################################
        part_f_sql = "SELECT movie_cast.cast_id AS cast_id, movie_cast.cast_name AS cast_name, printf(\"%.2f\", AVG(movies.score)) AS average_score FROM movies JOIN movie_cast ON movies.id = movie_cast.movie_id WHERE movie_cast.cast_id IN (SELECT movie_cast.cast_id FROM movie_cast JOIN movies ON movie_cast.movie_id = movies.id WHERE movies.score >= 25 GROUP BY movie_cast.cast_id HAVING COUNT(*) >= 3) AND movies.score >= 25 GROUP BY cast_id ORDER BY average_score DESC, cast_name ASC LIMIT 10"
        ######################################################################
    ...
    def part_g(self,connection):
        ############### EDIT SQL STATEMENT ###################################
        part_g_sql = "CREATE VIEW good_collaboration AS SELECT tab1.cast_id as cast_member_id1, tab2.cast_id as cast_member_id2, COUNT(*) as movie_count, AVG(movies.score) as average_movie_score FROM movie_cast tab1 JOIN movie_cast tab2 ON tab1.movie_id == tab2.movie_id AND tab1.cast_id < tab2.cast_id JOIN movies ON movies.id = tab1.movie_id GROUP BY tab1.cast_id, tab2.cast_id HAVING movie_count >=2 AND average_movie_score >=40" 
        ######################################################################
        return self.execute_query(connection, part_g_sql)
    
    def part_gi(self,connection):
        ############### EDIT SQL STATEMENT ###################################
        part_g_i_sql = "SELECT cast_member_id1 as cast_id, cast_bio.cast_name as cast_name, printf(\"%.2f\", AVG(average_movie_score)) as collaboration_score FROM (SELECT tab1.cast_id as cast_member_id1, tab2.cast_id as cast_member_id2, COUNT(*) as movie_count, AVG(movies.score) as average_movie_score FROM movie_cast tab1 JOIN movie_cast tab2 ON tab1.movie_id == tab2.movie_id AND tab1.cast_id <> tab2.cast_id JOIN movies ON movies.id = tab1.movie_id GROUP BY tab1.cast_id, tab2.cast_id HAVING movie_count >=2 AND average_movie_score >=40) AS subquary JOIN cast_bio ON cast_bio.cast_id = subquary.cast_member_id1 GROUP BY cast_id ORDER BY collaboration_score DESC, cast_bio.cast_name ASC LIMIT 5"
        ######################################################################
    ...
    def part_h(self,connection,path):
        ...
        file = csv.reader(open(path))
        for line in file:
            sql = "INSERT INTO movie_overview VALUES (?, ?)"
            connection.execute(sql, (line[0], line[1]))
        ######################################################################
